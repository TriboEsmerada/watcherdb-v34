...
class FileGroupAnalysis:
    ...
    disk_available_known: bool = True  # False = sem dado real de disco → não afirmar risco DISK (ABSTAIN)
...
class PredictiveAlertsEngine:
    ...
    async def analyze_server_alerts(self, server_id: str) -> Dict:
        ...
        try:
            ...
            # 1b. Espaço livre real por drive (query separada; ABSTAIN se indisponível)
            volume_free = await self._get_volume_free_gb(server_id)

            # 2. AGREGAR DATAFILES POR FILEGROUP
            filegroups = self._aggregate_by_filegroup(datafiles, volume_free)
    ...
    async def _get_volume_free_gb(self, server_id: str) -> Dict[str, float]:
        """
        Espaço livre REAL por drive (GB), via sys.dm_os_volume_stats da instância-alvo.

        Query SEPARADA com try/except próprio: se faltar VIEW SERVER STATE, devolve {}
        e o disco fica 'desconhecido' (ABSTAIN honesto) — os restantes alertas preditivos
        continuam a funcionar. Nunca fabricar um valor (lição cross_instance do council).
        """
        query = """
        SELECT
            LEFT(vs.volume_mount_point, 1) AS drive,
            MIN(vs.available_bytes) / 1073741824.0 AS available_gb
        FROM sys.master_files AS df
        CROSS APPLY sys.dm_os_volume_stats(df.database_id, df.file_id) AS vs
        GROUP BY LEFT(vs.volume_mount_point, 1)
        """
        try:
            result = await self.sql_monitoring.execute_query(server_id, query)
            if not result or not result.get('success'):
                err = result.get('error', 'desconhecido') if result else 'sem resposta'
                logger.warning(f"Sem dados de volume p/ {server_id}: {err} — disco fica desconhecido (ABSTAIN)")
                return {}
            out: Dict[str, float] = {}
            for row in result.get('rows', []):
                r = dict(row)
                drive = (r.get('drive') or '').upper()
                gb = r.get('available_gb')
                if drive and gb is not None:
                    out[drive] = float(gb)
            return out
        except Exception as e:
            logger.warning(f"dm_os_volume_stats indisponível em {server_id}: {e} — disco fica desconhecido (ABSTAIN)")
            return {}

    def _aggregate_by_filegroup(self, datafiles: List[Dict], volume_free: Dict[str, float] = None) -> Dict[str, FileGroupAnalysis]:
        """
        ...
        for (db_name, fg_name), files in grouped.items():
            ...
            # Espaço em disco REAL por drive (min entre os volumes deste filegroup).
            # Sem dado → desconhecido: NÃO fabricar (lição cross_instance do council).
            vols = {(f.get('volume') or 'C').upper() for f in files}
            avail = [volume_free[v] for v in vols if volume_free and v in volume_free]
            if avail:
                disk_available_gb = min(avail)
                disk_available_known = True
            else:
                disk_available_gb = 0.0
                disk_available_known = False
            ...
            # Determinar fator limitante (só afirmar DISK se houver dado real de disco)
            if has_unlimited:
                if disk_available_known:
                    limiting_factor = 'DISK'
                    limit_gb = disk_available_gb
                else:
                    limiting_factor = 'UNKNOWN'
                    limit_gb = 999999.0  # sem dado de disco: não inventar limite
            else:
                limit_gb = available_growth_gb
                if disk_available_known:
                    limiting_factor = 'MAXSIZE' if available_growth_gb < disk_available_gb else 'DISK'
                else:
                    limiting_factor = 'MAXSIZE'
        ...
        # Uso previsto em 30 dias
            ...
            if disk_available_known and disk_available_gb < 50 and days_until_full <= 30:
                has_overflow_risk = True
                risk_type = 'DISK'
            ...
            filegroups[fg_key] = FileGroupAnalysis(
                ...
                disk_available_known=disk_available_known,
    ...
    async def _analyze_filegroup_aggregated(self, fg: FileGroupAnalysis) -> List[PredictiveAlert]:
        ...
        # ========================================
        if fg.disk_available_known and fg.disk_available_gb > 0:
