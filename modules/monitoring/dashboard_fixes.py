...
class FileGroupDisplay:
    ...
    # Percentuais
    usage_percent: float
    maxsize_utilization_percent: float  # 0 para UNLIMITED
    
    # Flag UNLIMITED
    is_unlimited: bool  # True se algum arquivo do filegroup e UNLIMITED
...
class DashboardFixes:
    ...
    async def _get_filegroups_real_data(self, server_id: str) -> List[Dict]:
        ...
        # Nota: FILEPROPERTY não funciona no contexto do servidor (sys.master_files)
        # Precisamos usar estimativa (size * 0.75) ou buscar por database individual
        query = """
        -- Query para obter filegroups reais com agregação correta
        -- Usando estimativa de uso (75%) pois FILEPROPERTY requer USE [database]
        WITH FileGroupData AS (
            SELECT
                ...
                END AS max_size_mb,
                -- Flag para indicar se este arquivo e UNLIMITED
                CASE WHEN mf.max_size = -1 THEN 1 ELSE 0 END AS is_unlimited,
                -- Estimativa: 75% do tamanho alocado está em uso
                CAST(mf.size * 8.0 / 1024 * 0.75 AS DECIMAL(18,2)) AS used_mb,
        ...
        SELECT
            ...
            CASE
                WHEN MAX(is_unlimited) = 1 THEN -1
                ELSE CAST(SUM(max_size_mb) AS DECIMAL(18,2))
            END AS total_max_size_mb,
            MAX(is_unlimited) AS is_unlimited,
            -- Volume e disco
            volume,
    ...
    def _process_filegroup_data(self, fg_data: Dict) -> FileGroupDisplay:
        ...
        is_unlimited_flag = safe_int(fg_data.get('is_unlimited', 0))
        
        # Detectar UNLIMITED: max_size_mb = -1 ou flag is_unlimited = 1
        is_unlimited = (total_max_size_mb < 0) or (is_unlimited_flag == 1)
        ...
        # MAXSIZE: Se UNLIMITED, retorna -1 para indicar
        if is_unlimited:
            max_size_gb = -1
        else:
        ...
        # CORRIGIDO: maxsize_utilization_percent = 0 para UNLIMITED
        if is_unlimited:
            maxsize_utilization_percent = 0.0
        elif max_size_gb > 0:
            maxsize_utilization_percent = (total_size_gb / max_size_gb * 100)
        else:
            maxsize_utilization_percent = 0.0
        ...
        # Analise de risco (nao aplicavel para UNLIMITED)
        if is_unlimited:
            is_overflow = False  # UNLIMITED nao pode estar em overflow por maxsize
        else:
            # Verificar se soma dos maxsizes > espaco livre total
            is_overflow = total_maxsize_gb > total_disk_free_gb
        
        # REMOVIDO bloco duplicadoco livre total duplicado removido
        ...
        # CORRIGIDO: Thresholds 85%/95% baseado em maxsize_utilization_percent
        # UNLIMITED files nao geram alertas (maxsize_utilization_percent = 0)
        if is_overflow:
            alert_level = 'OVERFLOW'
            risk_factors = ['Espaço esgotado']
        elif not is_unlimited and maxsize_utilization_percent >= 95:
            alert_level = 'CRITICAL'
            risk_factors = ['Uso de maxsize critico (>=95%)']
        elif not is_unlimited and maxsize_utilization_percent >= 85:
            alert_level = 'WARNING'
            risk_factors = ['Uso de maxsize alto (>=85%)']
        else:
            alert_level = 'OK'
            risk_factors = []
        ...
        return FileGroupDisplay(
            ...
            is_unlimited=is_unlimited,  # Flag UNLIMITED
