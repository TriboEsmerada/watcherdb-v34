...
## [2026-09-16] watcherdb-v1-intel-specialist -> orchestrator + v3.4-council

[PROACTIVE FINDING]: data-integrity | `WATCHERDB INTELLIGENCE V1/database/INSTALACAO_COMPLETA_UNIFICADA.sql:14493-14626` (SECAO 30) | critical — canonico V1, activamente mantido (migrations 013-015 anexadas 2026-09-15), ainda cria `WatcherDB_Users` sem `must_change_password`/`local_password_hash`/`local_password_set_at`/`password_changed_at` E semeia 4 contas (`admin`,`salomao`,`ricardo` com role=admin; `viewer`) com hash bcrypt fixo de `admin123`, sem qualquer mecanismo de forcar troca. E exactamente o padrao que o commit `5ed4f68` (V3.4) removeu do lado do canonico V3.4 (SECAO 13) — o fork V1 nunca foi sincronizado. Fresh install a partir do script V1 cria 3 contas admin com password conhecida publicamente (o proprio ficheiro documenta o hash). Sugestao: migration `016_sync_users_auth_columns.sql` em `V1/database/migrations/` — remover os INSERTs de seed (alinhar com a decisao ja tomada em 5ed4f68) + acrescentar bloco idempotente das 4 colunas (copiar forma EXEC-wrapped de V3.4 `INSTALACAO_COMPLETA_UNIFICADA.sql:10136-10141`, nao a forma inline que falha — ver achado separado). Sem VETO formal (nada foi proposto que piore isto), mas achado de severidade critica que deve ser tratado como P1.

[PROACTIVE FINDING]: reliability | `WATCHERDB_V3.4/database/07_ADD_MUST_CHANGE_PASSWORD.sql` | high — `ALTER TABLE ADD must_change_password` seguido de `UPDATE` que referencia essa coluna, no MESMO batch (sem `GO` entre eles). SQL Server resolve nomes de coluna contra o catalogo tal como estava ANTES da batch correr — o `UPDATE` falha a compilar com Msg 207 (Invalid column name), a batch inteira aborta e o `ALTER` nunca chega a executar. Explica por que os irmaos 12/13 (so ALTER, sem DML a seguir na mesma batch) resultaram e este nao. Fix: `GO` a seguir ao `ALTER TABLE ADD`, ou envolver o `UPDATE` em `EXEC()`/`sp_executesql` (padrao ja usado no bloco canonico V3.4 10136-10141).
