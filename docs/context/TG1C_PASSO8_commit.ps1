# TestGrapete TG-1c - PASSO 8: semantico completo na 8434 e commit do que ficou fora (smoke+contexto, nightly,
# plano, scripts TG-1c/TG-2/reset). PS7. Corre TU. Pre-requisito: PASSO 7 aplicado. Espera 65 s antes do semantico (rate limit).
...
if (-not (Select-String -Path tests/e2e/test_smoke_modules_e2e.py -Pattern 'TG-1c PASSO 7' -Quiet)) { throw "PASSO 7 nao aplicado. ABORT." }
...
git add tests/e2e/test_semantic_e2e.py tests/e2e/test_smoke_modules_e2e.py scripts/qa/nightly_testgrapete.ps1 `
    ...
    docs/context/TG1C_PASSO7_fix_apply.py docs/context/TG1C_PASSO8_commit.ps1 `
...
'@
$msgFile = Join-Path $env:TEMP 'tg1c_p8_msg.txt'
