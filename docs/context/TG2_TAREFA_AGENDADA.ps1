# TestGrapete TG-2: regista a tarefa agendada que corre o job noturno na 8434. PS7, como ADMIN local. Corre TU.
...
# Verificar: Get-ScheduledTask -TaskName 'WatcherDB-TestGrapete' | Get-ScheduledTaskInfo ; docs\qa\externo\NIGHTLY_LOG.md
#Requires -Version 7
#Requires -RunAsAdministrator

$repo = 'C:\Users\ue_e-snetto\Documents\projetosPython\WATCHERDB_V3.4'
$nome = 'WatcherDB-TestGrapete'
$hora = '02:00'
...
if (Get-ScheduledTask -TaskName $nome -ErrorAction SilentlyContinue) {
    ...
    Register-ScheduledTask -TaskName $nome -Action $action -Trigger $trigger -Settings $settings -Principal $principal `
        -Description 'TestGrapete: smoke do portal (council) + scripts do qa-externo, contra 8434. Bundle em docs/qa/externo/<dia>/' | Out-Null
