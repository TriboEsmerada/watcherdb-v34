"""TestGrapete TG-1b - PASSO 1: escreve o gerador do painel estatico
(scripts/qa/testgrapete_board.py). Se o script noturno ja existir sem a chamada
ao painel (TG-1 aplicado antes desta adenda), acrescenta-a.

Uso (raiz do repo):  py docs/context/TG1B_PASSO1_apply.py
Depois:              pwsh docs/context/TG1B_PASSO2_commit.ps1
"""
...
MARK = "TESTGRAPETE TG-1b"
BOARD_PATH = ROOT / "scripts" / "qa" / "testgrapete_board.py"
NIGHTLY_PATH = ROOT / "scripts" / "qa" / "nightly_testgrapete.ps1"

BOARD_SRC = r'''"""TESTGRAPETE TG-1b - painel estatico das corridas (council afirma, qa-externo confere).
...
NIGHTLY_OLD = "Write-Host $logLine\nexit $councilExit\n"
NIGHTLY_NEW = (
...
def main() -> None:
    compile(BOARD_SRC, str(BOARD_PATH), "exec")
    if BOARD_PATH.exists():
        if MARK not in BOARD_PATH.read_text(encoding="utf-8", errors="replace"):
            sys.exit(f"ABORT: {BOARD_PATH.relative_to(ROOT)} existe sem marcador TG-1b.")
        print("Ja existe: scripts/qa/testgrapete_board.py")
    else:
        BOARD_PATH.parent.mkdir(parents=True, exist_ok=True)
        BOARD_PATH.write_text(BOARD_SRC, encoding="utf-8", newline="\n")
        print("OK: scripts/qa/testgrapete_board.py")

    if NIGHTLY_PATH.exists():
        n = NIGHTLY_PATH.read_text(encoding="utf-8")
        if "testgrapete_board.py" in n:
            print("Ja existe: chamada ao painel no script noturno")
        elif n.count(NIGHTLY_OLD) == 1:
            NIGHTLY_PATH.write_text(n.replace(NIGHTLY_OLD, NIGHTLY_NEW), encoding="utf-8", newline="\n")
            print("OK: scripts/qa/nightly_testgrapete.ps1 (+ chamada ao painel)")
        else:
            sys.exit("ABORT: nightly_testgrapete.ps1 nao tem o bloco final esperado; patch manual.")
    else:
        print("Nota: nightly_testgrapete.ps1 ainda nao existe (TG-1 PASSO 1 ja inclui a chamada ao painel).")
    print("Proximo: pwsh docs/context/TG1B_PASSO2_commit.ps1")
