...
**Uso:**
```bash
python test_parametrized_queries.py SQLHDSPRD013 --instance I03 --database TENT_TAP
```
...
```bash
# Histórico de crescimento - TENT_TAP
curl http://localhost:8000/api/queries/filegroup-growth-history/SQLHDSPRD013_I03?database=TENT_TAP

# Projeção de crescimento - TENT_TAP
curl http://localhost:8000/api/queries/filegroup-growth-forecast/SQLHDSPRD013_I03?database=TENT_TAP

# Índices faltantes - TENT_TAP
curl http://localhost:8000/api/queries/missing-index-analysis/SQLHDSPRD013_I03?database=TENT_TAP
```
...
query = SQLQueries.get_filegroup_growth_history_filtered('TENT_TAP')
result = await execute_query_on_server(server_id, query)
```
